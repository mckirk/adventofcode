...
class Input:
    ...
    @classmethod
    def from_file(cls, path: Path, spec_name: str | None = None, *, raw = False):
        if not path.is_file():
            return cls(None)
        else:
...
def get_problem_input(*, raw = False):
    return Input.from_file(script_dir / "input.txt", default_spec_name, raw=raw)


@cache
def get_sample_input(idx: int, *, raw = False):
    return Input.from_file(script_dir / f"sample{idx}.txt", default_spec_name, raw=raw)
