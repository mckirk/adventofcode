...
rooms_pos = [[(x,y) for y in [1,2]] for x in [2,4,6,8]]
hallway_pos = [(i, 0) for i in range(11)]

start_pos = [(x,y) for y in [1,2] for x in [2,4,6,8]]
...
class AnthroConfig:
    ...
    def possible_resulting_configs(self):
        ...
        for piece,room in enumerate(self.rooms_pieces):
            if len(room) == 1:
                occ_piece, occ_pos = room[0]
                if occ_piece == piece and occ_pos[1] == 2:
                    free_room_pos.append([(occ_pos[0], 1)])
                else:
                    free_room_pos.append([])
            elif len(room) == 0:
                free_room_pos.append(rooms_pos[piece])
            else:
                free_room_pos.append([])
...
target_config = AnthroConfig.from_set(frozenset(zip([0, 1, 2, 3, 0, 1, 2, 3,], start_pos)))
...
def main():
    ...
    print(f"Puzzle1: {energy_required(init_conf, target_config)}")
