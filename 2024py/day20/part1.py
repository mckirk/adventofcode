...
def run(inp: Input):
    ...
    for p, c in inp.as_pos:
        ...
        for np, _ in p.adj(limits=mp.limits, diagonal=False):
            for np2, _ in np.adj(limits=mp.limits, diagonal=False):
                if np2 in blocked:
                    continue
                potential_cheats.add((p, np2))

    counter = Counter()
    res = 0
    for cheat_start, cheat_end in potential_cheats:
        if cheat_start not in dist_s or cheat_end not in dist_e:
            continue
        dist = dist_s[cheat_start] + 2 + dist_e[cheat_end]
        saved = legit - dist
